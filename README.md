...
当前阶段已实现教师数据面板和学习数据统计展示。教师可以通过 mock 数据查看关卡通过率、错误类型分布、知识点掌握情况和教学建议。
...
第七阶段新增了前端错误诊断系统。运行失败或提交失败时，结果面板会显示“错误诊断”区域，用中文解释错误原因，原始 `stderr` 会保留在“原始错误信息”区域，方便需要时继续查看 gcc 输出。
...
错误诊断会识别一些常见 C 语言初学者错误：

- `scanf("%d", x)`：提示普通变量通常需要写成 `scanf("%d", &x)`。
- `if (x = 0)`：提示 `=` 是赋值，`==` 才是判断相等。
- gcc 输出包含 `expected ';'`：提示可能缺少分号。
- `cmd == "start"`：提示 C 字符串不能直接用 `==` 比较内容，应该使用 `strcmp`。
- 访问 `a[5]`：提示 5 个元素数组的合法下标是 0 到 4。
- `while(1)` 或 `for(;;)` 且运行超时：提示存在明显无限循环，需要设置退出条件。

错题本使用浏览器 `localStorage` 保存提交失败记录，不引入数据库。只保存“提交失败”，普通运行失败不会写入错题本。
...
- 最多保留 50 条。
- 刷新页面后记录仍然存在。
- `/mistakes` 页面支持展开查看代码和诊断、删除单条记录、清空错题本。

## 教师数据面板

第八阶段新增 `/teacher-dashboard` 教师数据面板。当前面板使用前端 mock 数据，不连接真实数据库、不接 AI、不包含登录系统。

面板展示的统计指标包括：

- 班级总览：学生人数、总提交次数、平均通过率、最薄弱知识点。
- 关卡通过率：5 个 MVP 关卡的通过率、平均分、平均提交次数。
- 错误类型分布：`compile_error`、`wrong_answer`、`timeout`、`runtime_error`。
- 知识点掌握情况：`printf 输出`、`scanf 输入`、`if else 判断`、`for 循环`、`数组最大值`。
- 教学建议：根据 mock 数据中最常见错误自动给出课堂讲解建议。

教学价值：

- 帮助教师快速看到学生在哪些关卡卡住。
- 帮助教师区分语法错误、答案错误、超时和运行错误。
- 帮助教师针对薄弱知识点安排复讲内容。
- 适合课堂答辩展示项目的数据闭环能力。
...
- 前端已展示运行结果和提交判题结果
- 运行失败和提交失败时展示中文错误诊断
- 根据常见 C 语言错误模式给出更具体提示
- 提交失败后自动保存错题记录到 localStorage
- 新增 `/mistakes` 错题本页面，支持查看、展开、删除和清空记录
- 新增 `/teacher-dashboard` 教师数据面板页面
- 使用前端 mock 数据展示学生人数、提交次数、平均通过率和薄弱知识点
- 展示 5 个关卡的通过率、平均分、平均提交次数和提示使用情况
- 展示错误类型分布和知识点掌握情况
- 根据最常见错误自动展示教学建议
- 配置根目录 npm workspaces 和 `npm run dev`
- 添加基础测试，覆盖首页渲染、关卡接口、关卡页面、真实运行和提交交互、C 代码运行接口、自动判题接口、错误诊断和错题本
